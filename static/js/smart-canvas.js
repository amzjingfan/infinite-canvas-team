...
function clearSmartNodeTransientRunState(node, options={}){
    ...
    delete node.autodlTaskId;
    delete node.autodlStatus;
    delete node.vintedOperationId;
    delete node.vintedStatus;
...
function isEditableTarget(target){
    ...
    return !!el?.closest?.('input, textarea, select, option, [contenteditable="true"], .prompt-node-control, .prompt-input, .agent-panel, .agent-toggle');
}
function getAgentCanvasContext(ids){
    return window.CanvasAgent.contextFromNodes(canvasId, nodes, ids, mediaKindForItem);
}
function createCanvasAgentHost(){
    return {
        canvasId,
        clientId:smartClientId,
        selectedReferenceIds:() => selectedNodeIds(),
        getAgentCanvasContext,
        getProviders:() => apiProviders,
        closeAssets:() => toggleAssetLibrary(false),
        applyAgentCanvasOperations,
        runCanvasAgentGeneration,
        resumeCanvasAgentGeneration,
        applyAgentGenerationResult,
        setAgentRunState,
        async ensureSaved(ids=[]){
            if(!canvasId || !canvas) throw new Error('画布尚未加载，请稍后重试。');
            ids = ids.slice();
            const expected = getAgentCanvasContext(ids);
            const saved = await queueCanvasSave();
            const persisted = window.CanvasAgent.contextFromNodes(canvasId, saved.nodes || [], ids, mediaKindForItem);
            if(JSON.stringify(expected) !== JSON.stringify(persisted) || JSON.stringify(expected) !== JSON.stringify(getAgentCanvasContext(ids))){
                throw new Error('引用节点尚未保存或保存期间发生变化，请重试。');
            }
            return persisted;
        }
    };
}
const pendingAgentStatuses = new Map();
function agentOperationNodes(conversationId, runId){
    const mapping = Object.create(null);
    nodes.forEach(node => {
        const owner = node.agent;
        if(owner?.canvasId === canvasId && owner.conversationId === conversationId && owner.runId === runId && !owner.displayOnly){
            mapping[owner.operationId] = node.id;
        }
    });
    return mapping;
}
function agentNodePosition(referenceIds, box={w:316,h:240}){
    const refs = referenceIds.map(id => nodes.find(node => node.id === id)).filter(Boolean).map(nodeRect);
    const center = viewportCenter();
    const x = refs.length ? Math.max(...refs.map(rect => rect.x + rect.width)) + 80 : center.x;
    let y = refs.length ? Math.min(...refs.map(rect => rect.y)) : center.y;
    const rects = nodes.map(nodeRect);
    let collision;
    while((collision = rects.find(rect => x < rect.x + rect.width + 28 && x + box.w + 28 > rect.x && y < rect.y + rect.height + 28 && y + box.h + 28 > rect.y))){
        y = collision.y + collision.height + 28;
    }
    return {x,y};
}
function agentGenerationMeta(source, operation, context){
    const confirmed = operation.settings;
    const video = source.outputKind === 'video';
    const runSettings = {engine:'api',apiKind:video ? 'video' : 'image',count:confirmed.count,
        ratio:Object.keys(API_RATIO_VALUES).find(key => API_RATIO_VALUES[key] === confirmed.aspect_ratio) || 'custom',
        customRatio:confirmed.aspect_ratio,resolution:confirmed.resolution,customSize:confirmed.size};
    if(video){
        Object.assign(runSettings, {videoProvider:confirmed.provider,videoModel:confirmed.model,
            videoDuration:confirmed.duration,videoResolution:confirmed.resolution,videoAspect:confirmed.aspect_ratio});
    } else {
        Object.assign(runSettings, {provider_id:confirmed.provider,model:confirmed.model,quality:confirmed.quality});
    }
    return {prompt:context.prompt,settings:runSettings,inputRefs:context.media,promptRefs:context.media,
        sourceNodeId:source.id,createdAt:Date.now()};
}
function applyAgentCanvasOperations({conversationId,runId,operations,run}){
    if(!canvasId || !canvas || !conversationId || !runId) throw new Error('Agent 运行标识缺失。');
    if(run?.mode === 'creation') return applyAgentCreativeTask({conversationId,runId,operations,run});
    const mapping = agentOperationNodes(conversationId, runId);
    const available = new Map(nodes.map(node => [node.id,node]));
    const owned = new Set(Object.values(mapping));
    const resolve = id => mapping[id] || id;
    // Validate the entire supplied ready batch before creating nodes or connections.
    const ready = operations.map(operation => {
        if(!/^[a-zA-Z0-9]{1,32}$/.test(operation.id) || !['create_prompt','create_media','connect','generate'].includes(operation.op)){
            throw new Error('不支持的 Agent 操作。');
        }
        const dependencies = operation.op === 'create_media' ? operation.reference_node_ids || []
            : operation.op === 'connect' ? [operation.from,operation.to]
            : operation.op === 'generate' ? [operation.node] : [];
        const refs = dependencies.map(resolve);
        refs.forEach(id => { if(!available.has(id)) throw Object.assign(new Error(`Agent 依赖节点不存在：${id}，请重新制定并确认计划。`),
            {code:'AGENT_DEPENDENCY_INVALID',operationId:operation.id}); });
        if((operation.op === 'connect' && !owned.has(refs[1])) || (operation.op === 'generate' && !owned.has(refs[0]))){
            throw Object.assign(new Error('Agent 只能修改本次运行创建的节点。请重新制定并确认计划。'),
                {code:'AGENT_DEPENDENCY_INVALID',operationId:operation.id});
        }
        let meta;
        if(operation.op === 'generate') {
            if(run?.id !== runId || run?.canvas_id !== canvasId || run?.conversation_id !== conversationId || run?.client_id !== smartClientId)
                throw new Error('Agent 冻结运行或所有权不匹配。');
            meta = agentFrozenGenerationMeta(run,operation.id);
        }
        if(operation.op !== 'connect'){
            const id = mapping[operation.id] || `agent_${runId}_${operation.id}`;
            if(available.has(id) && !owned.has(id)) throw new Error('Agent 节点标识冲突。');
            mapping[operation.id] = id;
            available.set(id, available.get(id) || {id});
            owned.add(id);
        }
        return {operation,refs,meta};
    });
    const created = [];
    ready.forEach(({operation,refs,meta}) => {
        if(operation.op === 'connect'){
            connectInputNode(refs[0], refs[1]);
            return;
        }
        const id = mapping[operation.id];
        created.push(id);
        if(nodes.some(node => node.id === id)) return;
        const generation = operation.op === 'generate';
        const source = generation ? nodes.find(node => node.id === refs[0]) : null;
        const count = generation ? operation.settings.count : 1;
        const ratioParts = String(operation.settings?.aspect_ratio || '1:1').split(':').map(Number);
        const ratio = ratioParts[0] > 0 && ratioParts[1] > 0 ? ratioParts[0] / ratioParts[1] : 1;
        const box = generation ? {w:Math.round(280 * Math.min(1,ratio)) * Math.min(count,2),h:Math.round(280 / Math.max(1,ratio)) * Math.ceil(count / 2)} : {w:316,h:240};
        const position = agentNodePosition(refs.length ? refs : selectedNodeIds(),box);
        const node = operation.op === 'create_prompt'
            ? createPromptNode(position.x,position.y,{select:false,skipUndo:true})
            : createNode(position.x,position.y,[],{select:false,skipUndo:true});
        // Constructors have not inserted links; assign identity before connecting.
        node.id = id;
        node.agent = {canvasId,conversationId,runId,operationId:operation.id,
            role:generation ? 'output' : operation.op === 'create_prompt' ? 'prompt' : 'source',status:'running'};
        if(operation.op === 'create_prompt') node.text = operation.text;
        else node.outputKind = generation ? meta.settings.apiKind : operation.kind;
        if(generation){
            Object.assign(node,{pending:count,running:true,w:box.w,h:box.h,runStartedAt:nowMs(),runTimerHidden:false});
            addConnection(source.id,node.id,'flow');
            attachRunMeta(node,meta);
        } else if(operation.op === 'create_media') refs.forEach(from => connectInputNode(from,node.id));
    });
    render();
    scheduleSave();
    return {created_node_ids:[...new Set(created)],operation_nodes:agentOperationNodes(conversationId,runId)};
}
function applyAgentCreativeTask({conversationId,runId,operations,run}){
    if(run.id !== runId || run.canvas_id !== canvasId || run.conversation_id !== conversationId || run.client_id !== smartClientId)
        throw new Error('Agent 冻结运行或所有权不匹配。');
    if(operations.length !== 1 || operations[0].op !== 'generate') throw new Error('独立创作只执行单个生成任务。');
    const operation = run.operations.find(op => op.id === operations[0].id && op.op === 'generate');
    if(!operation || !/^[a-zA-Z0-9]{1,32}$/.test(operation.id)) throw new Error('Agent 冻结创作任务不存在。');
    const id = `agent_${runId}_${operation.id}`, promptId = id+'_prompt';
    for(const nodeId of [id,promptId]) {
        const node = nodes.find(n=>n.id===nodeId), owner = node?.agent;
        if(node && (owner?.canvasId !== canvasId || owner.conversationId !== conversationId || owner.runId !== runId ||
            owner.operationId !== (nodeId === id ? operation.id : operation.id+'_prompt') ||
            owner.role !== (nodeId === id ? 'output' : 'prompt'))) throw new Error('Agent 节点标识冲突。');
    }
    const meta = agentFrozenGenerationMeta(run,operation.id);
    if(!nodes.some(n=>n.id===id)) {
        if(!nodes.some(n=>n.id===promptId)) {
            const position = agentNodePosition(operation.reference_node_ids);
            const prompt = createPromptNode(position.x,position.y,{select:false,skipUndo:true});
            prompt.id = promptId; prompt.text = operation.prompt;
            prompt.agent = {canvasId,conversationId,runId,operationId:operation.id+'_prompt',role:'prompt',displayOnly:true};
        }
        const [width,height] = operation.settings.aspect_ratio.split(':').map(Number);
        const ratio = width/height, count = operation.settings.count;
        const box = {w:Math.round(280*Math.min(1,ratio))*Math.min(count,2),h:Math.round(280/Math.max(1,ratio))*Math.ceil(count/2)};
        const position = agentNodePosition([promptId],box);
        const output = createNode(position.x,position.y,[],{select:false,skipUndo:true});
        Object.assign(output,{id,outputKind:operation.kind,pending:count,running:true,w:box.w,h:box.h,
            runStartedAt:nowMs(),runTimerHidden:false,
            agent:{canvasId,conversationId,runId,operationId:operation.id,role:'output',status:run.status}});
        attachRunMeta(output,meta);
    }
    render(); scheduleSave();
    return {created_node_ids:[id],operation_nodes:agentOperationNodes(conversationId,runId)};
}
async function runCanvasAgentGeneration({conversationId,runId,operation,run,clientId=smartClientId}){
    const operationId = operation.id;
    applyAgentCanvasOperations({conversationId,runId,operations:[operation],run});
    const url = agentGenerationUrl(conversationId,runId,operationId);
    await createCanvasAgentHost().ensureSaved();
    return executeCanvasAgentStep(url,clientId,false);
}
function agentGenerationUrl(conversationId,runId,operationId){
    return `/api/canvases/${encodeURIComponent(canvasId)}/agent/conversations/${encodeURIComponent(conversationId)}/runs/${encodeURIComponent(runId)}/steps/${encodeURIComponent(operationId)}/execute`;
}
async function executeCanvasAgentStep(url,clientId,resumeOnly){
    const response = await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({client_id:clientId,resume_only:resumeOnly})});
    const data = await response.json();
    if(!response.ok) throw new Error(apiErrorMessage(data,'Agent 生成请求失败。'));
    return data;
}
async function resumeCanvasAgentGeneration({conversationId,runId,operation,clientId=smartClientId}){
    return executeCanvasAgentStep(agentGenerationUrl(conversationId,runId,operation.id),clientId,true);
}
function agentFrozenGenerationMeta(run, operationId){
    const operation = run.operations.find(op => op.id === operationId && op.op === 'generate');
    const context = window.CanvasAgent.generationInputs(run,operationId);
    const meta = agentGenerationMeta({id:`agent_${run.id}_${operation.node}`,outputKind:context.kind},operation,context);
    if(run.mode === 'creation') delete meta.sourceNodeId;
    return meta;
}
function applyAgentGenerationResult({conversationId,runId,operation,result,run}){
    const mapping = agentOperationNodes(conversationId,runId);
    let output = nodes.find(node => node.id === mapping[operation.id]);
    const step = run?.steps?.find(step => step.operation_id === operation.id);
    const frozenOperation = run?.operations?.find(op => op.id === operation.id && op.op === 'generate');
    if(run?.id !== runId || run?.canvas_id !== canvasId || run?.conversation_id !== conversationId ||
        run?.client_id !== smartClientId || !frozenOperation || !step?.result?.media?.length)
        throw new Error('Agent 冻结结果或所有权不匹配，请刷新运行状态。');
    result = step.result;
    if(!result?.media?.length || result.media.some(item => !item.url || !['image','video'].includes(item.kind))){
        throw new Error('Agent 生成结果无效。');
    }
    if(!output){
        const id = `agent_${runId}_${operation.id}`;
        if(nodes.some(node => node.id === id) || !['submitting','running','generated','unknown','failed'].includes(step.status)){
            throw new Error('Agent 输出节点不存在或恢复所有权不匹配，请刷新运行状态。');
        }
        const sourceId = run.mode === 'creation' ? null : mapping[frozenOperation.node];
        const position = agentNodePosition(sourceId ? [sourceId] : []);
        output = createNode(position.x,position.y,[],{select:false,skipUndo:true});
        output.id = id;
        output.outputKind = result.media[0].kind;
        output.agent = {canvasId,conversationId,runId,operationId:operation.id,role:'output',status:run.status};
        if(sourceId) addConnection(sourceId,id,'flow');
        mapping[operation.id] = id;
    }
    if(output.agent.role !== 'output') throw new Error('Agent 输出节点所有权不匹配。');
    const meta = agentFrozenGenerationMeta(run,operation.id);
    if(result.attempts?.length) {
        const best = window.CanvasAgent.generationResultContext(run,operation.id);
        Object.assign(meta,{prompt:best.prompt,inputRefs:best.media,promptRefs:best.media});
        output.agent.bestUrl = result.best_url || '';
        output.agent.qualityStatus = result.quality?.status || '';
    }
    if(!mapping[frozenOperation.node]) delete meta.sourceNodeId;
    attachRunMeta(output,meta);
    // Capture current selection only within this synchronous finalization, never across an await.
    const selection = {id:selectedId,image:selectedImage};
    try {
        finalizePendingNode(output,result.media,null,result.media[0].kind);
    } finally {
    ...
    render();
    scheduleSave();
    return {created_node_ids:[output.id],operation_nodes:mapping};
}
function setAgentRunState({conversationId,runId,status}){
    const ids = new Set(Object.values(agentOperationNodes(conversationId,runId)));
    nodes.forEach(node => {
        if(ids.has(node.id) && node.agent.status !== status){
            node.agent.status = status;
            pendingAgentStatuses.set(node.id,status);
        }
    });
    render();
    scheduleSave();
...
function smartMinimaxEngine(node){
    if(node?.minimaxEngine === 'autodl') return 'autodl';
...
function resolveChatModel(model='', providerId=''){
    ...
    return models.includes(model) ? model : (models[0] || model || 'gpt-6-astra');
}
...
function isVintedProviderId(id){
    return id === 'vinted' || /^https:\/\/vinted\.cam(?:\/|$)/.test((apiProviders || []).find(p => p.id === id)?.base_url || '');
}
function vintedDurations(model){
    return model === 'seedance2.5' ? [30] : model === 'seedance2.0mini' ? [5,10] : [5,10,15];
}
function renderVideoDurationControl(){
    if(isVintedProviderId(settings.videoProvider)){
        const options = vintedDurations(settings.videoModel);
        return `<div class="smart-control duration-control"><button class="smart-pill" type="button">${settings.videoDuration}s</button>
            <div class="smart-popover compact-popover"><div class="duration-grid">${options.map(n => `<button type="button" class="duration-option ${n === settings.videoDuration ? 'active' : ''}" data-smart-param="videoDuration" data-smart-value="${n}">${n}s</button>`).join('')}</div></div></div>`;
    }
    const max = settings.videoProvider === 'autodl' ? 15 : 60;
    const v = Math.max(1, Math.min(max, Number(settings.videoDuration) || 5));
...
function renderVideoAspectControl(){
    const options = isVintedProviderId(settings.videoProvider) ? ['16:9','9:16','1:1','4:3','3:4','21:9'].map(v => [v,v]) : settings.videoProvider === 'autodl' ? [['9:16','9:16'], ['16:9','16:9']] : [
...
function renderVideoResolutionControl(){
    const options = isVintedProviderId(settings.videoProvider) ? [['720p','720P']] : settings.videoProvider === 'autodl' ? [['480p','480P'], ['768p','768P']] : [['', tr('smart.videoResAuto')], ['480p','480P'], ['720p','720P'], ['1080p','1080P']];
...
function renderApiVideoParams(){
    ...
    if(isVintedProviderId(settings.videoProvider)){
        const durations = vintedDurations(settings.videoModel);
        if(!durations.includes(Number(settings.videoDuration))) settings.videoDuration = durations[0];
        settings.videoResolution = '720p';
        if(!['16:9','9:16','1:1','4:3','3:4','21:9'].includes(settings.videoAspect)) settings.videoAspect = '16:9';
        dynamicParams.innerHTML = `${renderVideoProviderControl(providers)}${renderVideoModelControl(models)}
            ${renderVideoResolutionControl()}${renderVideoAspectControl()}${renderVideoDurationControl()}
            ${renderVideoToggleControl('videoCameraFixed', tr('smart.videoCameraFixed'))}
            <div class="muted-note">Vinted · 文字/参考图片生视频 · 原画下载 · 不支持专用首尾帧、参考视频或音频。</div>`;
        return;
    }
    if(settings.videoProvider === 'autodl'){
        if(!['480p','768p'].includes(settings.videoResolution)) settings.videoResolution = '768p';
        if(!['9:16','16:9'].includes(settings.videoAspect)) settings.videoAspect = '9:16';
        settings.videoDuration = Math.max(1, Math.min(15, Math.round(Number(settings.videoDuration) || 5)));
        dynamicParams.innerHTML = `${renderVideoProviderControl(providers)}${renderVideoModelControl(models)}
            ${renderVideoResolutionControl()}${renderVideoAspectControl()}${renderVideoDurationControl()}
            <div class="muted-note">AutoDL H3 · 最多 9 张图片、3 段音频，不支持参考视频。素材将上传至 Litterbox 临时托管（72 小时）。</div>`;
        return;
    }
    dynamicParams.innerHTML = `
...
let canvasSaveQueue = Promise.resolve();
...
function applyMergedServerCanvas(serverCanvas){
    ...
    nodes.forEach(node => {
        ...
        if(pendingAgentStatuses.has(node.id)) node.agent.status = pendingAgentStatuses.get(node.id);
        // Agent source inputs are represented by these same input links. The generic
        // node merge may choose an older server object while the link union retains additions.
        if(node.agent.role === 'source'){
            node.inputNodeIds = [...new Set(canvas.connections
                .filter(conn => conn.to === node.id && conn.kind === 'input').map(conn => conn.from))];
        }
    });
...
function toggleAssetLibrary(open=!assetLibraryOpen){
    ...
    if(open) window.CanvasAgent?.ui?.setOpen(false);
...
async function loadCanvas(){
    ...
    try {
        ...
        nodes.forEach(n => {
            ...
            if(n.agent && !n.agent.displayOnly && !['completed','failed'].includes(n.agent.status)) return;
            if(n.autodlTaskId && n.type !== 'smart-minimax') clearSmartNodeBusyState(n);
...
async function saveCanvas(){
    if(!canvasId || !canvas) return;
    try {
        await queueCanvasSave();
    } catch(error) {
        toast(error.message || '画布保存失败，请重试。');
    }
}
function queueCanvasSave(){
    const saving = canvasSaveQueue.then(persistCanvas);
    // A failed manual/Agent save must not poison later saves.
    canvasSaveQueue = saving.catch(() => {});
    return saving;
}
async function persistCanvas(){
    ...
    try {
        for(let attempt=0; attempt<3; attempt++){
            // Snapshot when this queued save executes, and again after each merge.
            savePromptDraftForCurrent();
            nodes.forEach(node => {
            ...
            if(res.ok){
                ...
                storageCanvas.nodes.forEach(node => {
                    if(node.agent && pendingAgentStatuses.get(node.id) === node.agent.status){
                        pendingAgentStatuses.delete(node.id);
                    }
                });
                return storageCanvas;
            }
            if(res.status !== 409) throw new Error(apiErrorMessage(data,'画布保存失败，请重试。'));
            if(data.detail?.canvas) applyMergedServerCanvas(data.detail.canvas);
            else if(data.detail?.updated_at) canvas.updated_at = data.detail.updated_at;
        }
        throw new Error('画布保存冲突，请重试。');
    } finally {
...
function createMinimaxNode(x, y, options={}){
    ...
    const node = {
        ...
        minimaxEngine:'autodl',
        autodlResolution:'768p竖',
    ...
    return `<div class="minimax-card minimax-workbench">
        <div class="minimax-wb-toolbar">
            <div class="minimax-brand">
                ...
                <span>${minimaxEngine === 'autodl' ? 'MiniMax H3 · AutoDL 视频生成' : 'MiniMax H3'}</span>
                    ...
                    <div class="minimax-clip-parameters">
                        ...
                        <div class="minimax-settings minimax-segment-fields">
                            <label class="minimax-wide-setting minimax-engine-setting"><span>Engine</span><select class="minimax-engine-select" data-minimax-engine title="选择生成来源">
                                ...
                                <option value="autodl" ${minimaxEngine === 'autodl' ? 'selected' : ''}>AutoDL · H3 视频生成</option>
                            </select></label>
                            <label><span>Duration</span><input type="number" min="${minimaxEngine === 'autodl' ? 1 : 0.5}" max="${minimaxEngine === 'autodl' ? 15 : 60}" step="${minimaxEngine === 'autodl' ? 1 : 0.1}" data-minimax-seg-number="duration" value="${escapeAttr(segDuration)}"><b>s</b></label>
                            ${minimaxEngine === 'autodl' ? `<label class="minimax-wide-setting"><span>分辨率</span><select data-minimax-select="autodlResolution">${['480p竖','768p竖','480p横','768p横'].map(value => `<option ${value === (selected?.autodlResolution || node.autodlResolution || '768p竖') ? 'selected' : ''}>${value}</option>`).join('')}</select></label>
                            <label class="minimax-wide-setting"><span>随机种子（可选）</span><input type="number" step="1" data-autodl-seed value="${escapeAttr(selected?.autodlSeed ?? '')}" placeholder="留空随机"></label>
                            <label class="minimax-wide-setting"><span>AutoDL 密钥</span><input type="password" data-autodl-key autocomplete="off" placeholder="已配置可留空；更换后点击保存"><button type="button" data-autodl-save>保存</button></label>
                            <div class="minimax-wide-setting">图片与音频会上传至 Litterbox 临时托管（72 小时），供 AutoDL 读取。不支持参考视频。${escapeHtml(node.autodlStatus || '')}${selected?.autodlTaskId ? ` · 任务 ${escapeHtml(selected.autodlTaskId)}（再次点击生成可继续查询）` : ''}</div>` : ''}
                            ${minimaxEngine !== 'autodl' ? `
                            ...
                            ` : ''}
                            <button class="minimax-run ${node.running ? 'is-stop' : ''}" type="button" data-minimax-run="${escapeAttr(node.id)}" ${node.running ? 'disabled' : ''} title="Generate selected clip"><i data-lucide="${node.running ? 'loader-2' : 'sparkles'}"></i><span>${minimaxEngine === 'autodl' ? (node.running ? '处理中' : selected?.autodlTaskId ? '继续查询' : '生成视频') : node.running ? 'Running' : 'Generate clip'}</span></button>
                        </div>
                    </div>
                </div>
            </div>
...
function nodeBodyHtml(node, layout){
    ...
    if((node.autodlStatus || node.vintedOperationId) && imgs.length === 0){
        return autodlPendingBodyHtml(node, layout);
    }
...
function autodlPendingBodyHtml(node, layout){
    if(node.vintedOperationId){
        return `<div class="jimeng-pending-cell loading-cell single" style="width:${layout.width}px;height:${layout.height}px"><div class="jimeng-pending-overlay">
            <div class="jimeng-pending-text">Vinted · ${escapeHtml(node.vintedStatus || '任务已保存')}</div>
            <button type="button" class="jimeng-pending-query" data-vinted-query="${escapeAttr(node.id)}" ${node.running ? 'disabled' : ''}>查询结果</button></div></div>`;
    }
    return `<div class="jimeng-pending-cell loading-cell single" style="width:${layout.width}px;height:${layout.height}px">
        <div class="jimeng-pending-overlay">
            <div class="jimeng-pending-text">AutoDL H3 · ${escapeHtml(node.autodlStatus || '任务已保存')}</div>
            <div class="jimeng-pending-sub">${escapeHtml(node.autodlTaskId || '')}</div>
            ${node.autodlTaskId ? `<button class="jimeng-pending-query" type="button" data-autodl-query="${escapeAttr(node.id)}" ${node.running ? 'disabled' : ''}>${node.running ? '查询中…' : '查询结果'}</button>` : ''}
        </div>
    </div>`;
}
...
function render(){
    ...
    const nodeHtmlEntries = nodes
        ...
        const html = `<div class="image-node ${isEmpty ? 'empty-node' : ''} ${isGroup ? 'group-node' : ''} ${isHistory ? 'history-group-node' : ''} ${isPrompt ? 'prompt-smart-node' : ''} ${isLoop ? 'loop-smart-node' : ''} ${isMinimax ? 'minimax-smart-node' : ''} ${isSmartGroup ? 'smart-group-node' : ''} ${isCompactMember ? 'smart-group-member-node' : ''} ${isNodeSelected(node.id) ? 'selected' : ''} ${(dragState?.groupIds?.includes(node.id) || dragState?.id === node.id) ? 'dragging' : ''} ${node.running ? 'node-running' : ''} ${isPending ? 'node-pending' : ''}" data-id="${escapeHtml(node.id)}" style="left:${node.x || 0}px;top:${node.y || 0}px;width:${layout.width}px;height:${layout.height}px">
            ...
            ${isPrompt || imgs.length ? '<button class="agent-node-reference" type="button" title="引用此节点的内容到 Agent 对话" aria-label="引用此节点到 Agent"><i data-lucide="quote"></i><span>引用</span></button>' : ''}
...
function bindMinimaxNodeControls(el, node){
    ...
    el.querySelectorAll('[data-minimax-engine]').forEach(select => {
        const seedInput = el.querySelector('[data-autodl-seed]');
        if(seedInput) seedInput.onchange = e => {
            e.stopPropagation();
            const seg = smartMinimaxSelectedSegment(node);
            if(seg) seg.autodlSeed = seedInput.value === '' ? null : Number(seedInput.value);
            scheduleSave();
        };
        const save = el.querySelector('[data-autodl-save]');
        if(save) save.onclick = async e => {
            e.stopPropagation();
            const input = el.querySelector('[data-autodl-key]');
            try {
                await autodlFetch('/api/autodl/settings', {api_key:input.value});
                input.value = '';
                toast('AutoDL 密钥已保存');
        ...
        select.onchange = e => {
            e.stopPropagation();
            focusMinimaxNode();
            const nextEngine = ['runninghub', 'autodl'].includes(select.value) ? select.value : SMART_MINIMAX_DEFAULT_ENGINE;
...
function bindNodeEvents(){
    world.querySelectorAll('.image-node').forEach(el => {
        ...
        el.querySelectorAll('.agent-node-reference').forEach(button => {
            ['pointerdown','mousedown','dblclick'].forEach(type => button.addEventListener(type, event => {
                event.stopPropagation();
            }));
            button.addEventListener('click', async event => {
                event.preventDefault(); event.stopPropagation();
                const agent = window.CanvasAgent?.ui;
                if(!agent) return;
                button.disabled = true;
                try { await agent.referenceNodes([id]); }
        ...
        el.querySelectorAll('[data-autodl-query]').forEach(btn => {
            btn.addEventListener('mousedown', e => { e.preventDefault(); e.stopPropagation(); }, true);
            btn.addEventListener('click', e => {
                e.preventDefault(); e.stopPropagation();
                resumeAutoDLVideoNode(btn.dataset.autodlQuery);
            });
        });
        el.querySelectorAll('[data-vinted-query]').forEach(btn => {
            btn.addEventListener('mousedown', e => { e.preventDefault(); e.stopPropagation(); }, true);
            btn.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); resumeVintedVideoNode(btn.dataset.vintedQuery); });
        });
...
function finalizePendingNode(pendingNode, urls, meta, kind='image'){
    ...
    if(imgs.length && kind === 'video') delete pendingNode.vintedOperationId;
...
async function runGeneration(){
    const node = selectedNode();
    if(node?.agent && !node.agent.displayOnly && !['completed','failed'].includes(node.agent.status)){
        toast('此节点由 Agent 运行管理，请在 Agent 面板查看进度。');
        return;
    }
    if(node?.agent?.status === 'failed'){
        // An explicit manual retry takes over a known failed output. Release once,
        // so subsequent clicks cannot clear the new manual generation's busy state.
        clearSmartNodeBusyState(node);
        delete node.agent;
    }
    if(node?.type === 'smart-minimax') return runMinimaxNode(node.id);
    if(node?.autodlTaskId) return resumeAutoDLVideoNode(node.id);
    if(node?.vintedOperationId) return resumeVintedVideoNode(node.id);
    const request = buildPromptRequest(node, null, true, smartLoopContext);
    const prompt = request.prompt.trim();
    if(!node) return;
    ...
    const autodlVideoRun = isApiLikeEngine(settings.engine) && settings.apiKind === 'video' && settings.videoProvider === 'autodl';
    const apiConcurrentRun = !autodlVideoRun && (isApiLikeEngine(settings.engine) || settings.engine === 'runninghub' || settings.engine === 'modelscope' || settings.engine === 'comfy');
    ...
    try {
        ...
        settings = previousSettings;
        if(liveSmartNode(pendingNode).autodlTaskId || liveSmartNode(pendingNode).vintedOperationId){
            const savedNode = liveSmartNode(pendingNode);
            savedNode.pending = 0;
            savedNode.running = false;
            scheduleSave();
            toast(e.message || '任务已保存，可在输出节点继续查询');
            return;
        }
...
async function resumeAutoDLVideoNode(nodeId){
    let node = nodes.find(item => item.id === nodeId);
    if(!node?.autodlTaskId || node.running) return;
    node.running = true;
    render();
    try {
        const urls = await runAutoDLVideoGeneration('', [], {}, node);
        finalizePendingNode(node, urls, null, 'video');
    } catch(error){
        toast(error.message || 'AutoDL 查询失败');
    } finally {
        node = liveSmartNode(node);
        node.running = false;
        node.pending = 0;
...
async function runAutoDLVideoGeneration(prompt, refs, runSettings, node){
    let state = liveSmartNode(node) || {};
    if(!state.autodlTaskId){
        const duration = Number(runSettings.videoDuration || 5);
        const resolution = runSettings.videoResolution || '768p';
        const aspect = runSettings.videoAspect || '9:16';
        const images = imageRefsOnly(refs).map(ref => ref.url);
        const audios = audioRefsOnly(refs).map(ref => ref.url);
        if(!String(prompt || '').trim() || prompt.length > 10000) throw new Error('提示词需要 1–10000 个字符');
        if(!Number.isInteger(duration) || duration < 1 || duration > 15) throw new Error('AutoDL 时长必须为 1–15 秒整数');
        if(!['480p','768p'].includes(resolution) || !['9:16','16:9'].includes(aspect)) throw new Error('请选择 AutoDL 支持的分辨率和比例');
        if(images.length > 9 || audios.length > 3) throw new Error('最多 9 张图片、3 段音频');
        if(videoRefsOnly(refs).length) throw new Error('此 API 不支持参考视频，请移除视频参考');
        state.autodlStatus = '上传素材并提交任务…';
        render();
        const task = await autodlFetch('/api/autodl/tasks', {prompt, duration, resolution:resolution + (aspect === '9:16' ? '竖' : '横'), images, audios});
        state = liveSmartNode(state);
        state.autodlTaskId = task.task_id;
        scheduleSave();
    }
    const deadline = Date.now() + 30 * 60 * 1000;
    try {
        while(Date.now() < deadline){
            const task = await autodlFetch(`/api/autodl/tasks/${encodeURIComponent(state.autodlTaskId)}`);
            state = liveSmartNode(state);
            state.autodlStatus = ({QUEUED:'排队中', RUNNING:'生成中', SUCCESS:'下载视频…', COMPLETED:'下载视频…'})[task.status] || task.status;
            render();
            if(['SUCCESS','COMPLETED'].includes(task.status)){
                const result = await autodlFetch('/api/autodl/download', {task_id:state.autodlTaskId});
                state = liveSmartNode(state);
                delete state.autodlTaskId;
                state.autodlStatus = '已完成';
                return result.urls;
            }
            if(task.status === 'FAILED'){
                delete state.autodlTaskId;
                throw new Error(task.message || 'AutoDL 生成失败');
            }
        ...
        state = liveSmartNode(state);
        state.autodlStatus = state.autodlTaskId ? '查询中断，点击查询结果可继续' : '生成失败';
        if(state.autodlTaskId) error.message += `；任务 ${state.autodlTaskId} 已保存，可在输出节点继续查询`;
        throw error;
    } finally {
        scheduleSave();
    }
}
async function runVintedVideoGeneration(prompt, refs, runSettings, node){
    let state = liveSmartNode(node);
    if(!state) throw new Error('请从画布节点提交 Vinted 视频，以便保留任务记录');
    ...
    const operation = state.vintedOperationId;
    const url = resume ? `/api/vinted/operations/${operation}/resume` : '/api/canvas-video';
    const payload = resume ? {} : {provider_id:runSettings.videoProvider, vinted_operation_id:operation,
        prompt, model:runSettings.videoModel, duration:Number(runSettings.videoDuration || 5),
        aspect_ratio:runSettings.videoAspect || '16:9', resolution:'720p',
        images:refs.map(ref => ({url:ref.url,name:ref.name || ''})), camerafixed:!!runSettings.videoCameraFixed};
    try {
        const response = await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
        const data = await response.json();
        if(!response.ok){
            if(data.detail?.terminal || (!resume && [400,422].includes(response.status))) delete liveSmartNode(state).vintedOperationId;
            throw new Error(typeof data.detail === 'string' ? data.detail : data.detail?.message || 'Vinted 请求未完成');
        }
        // Keep the operation until the caller saves the actual result to the node.
        state = liveSmartNode(state);
        state.vintedStatus = '已返回';
        return resultMediaUrls(data);
    } catch(error){
        liveSmartNode(state).vintedStatus = '查询中断，可点击查询结果继续';
        throw error;
    }
}
async function resumeVintedVideoNode(nodeId){
    let node = nodes.find(n => n.id === nodeId);
    if(!node?.vintedOperationId || node.running) return;
    node.running = true; render();
    try {
        const result = await runVintedVideoGeneration('', [], {}, node);
        node = liveSmartNode(node);
        finalizePendingNode(node, result, null, 'video');
        delete node.vintedOperationId;
        await queueCanvasSave();
    } catch(error){ toast(error.message); }
    finally { node = liveSmartNode(node); node.running = false; node.pending = 0; scheduleSave(); render(); }
}
async function runApiVideoGeneration(prompt, refs, runSettings=settings, node=null){
    ...
    try {
        if(runSettings.videoProvider === 'autodl') return await runAutoDLVideoGeneration(prompt, refs, runSettings, node);
        if(isVintedProviderId(runSettings.videoProvider)) return await runVintedVideoGeneration(prompt, refs, runSettings, node);
...
function smartMinimaxRunSnapshot(node, extraSettings={}){
    ...
    return {
        ...
        settings:{
            engine:engine === 'autodl' ? 'autodl' : engine === 'runninghub' ? 'runninghub' : 'comfy',
...
async function autodlFetch(url, body){
...
async function runMinimaxAutoDL(node){
    const seg = smartMinimaxSelectedSegment(node);
    if(!seg) throw new Error('请先选择视频片段');
    if(!seg.autodlTaskId){
        const prompt = String(seg.prompt || node.promptDraftText || inputPromptTextFor(node) || '').trim();
        const duration = Number(seg.duration || node.duration || 5);
        const images = smartMinimaxRefsForKind(node, 'image').map(ref => ref.url);
        const audios = smartMinimaxRefsForKind(node, 'audio').map(ref => ref.url);
        if(!prompt || prompt.length > 10000) throw new Error('提示词需要 1–10000 个字符');
        if(!Number.isInteger(duration) || duration < 1 || duration > 15) throw new Error('AutoDL 时长必须为 1–15 秒整数');
        if(images.length > 9 || audios.length > 3) throw new Error('最多 9 张图片、3 段音频');
        if(smartMinimaxRefsForKind(node, 'video').length) throw new Error('此 API 不支持参考视频，请移除视频参考');
        node.autodlStatus = '上传素材并提交任务…';
        render();
        const task = await autodlFetch('/api/autodl/tasks', {
            prompt, duration, images, audios, seed:seg.autodlSeed ?? null,
            resolution:seg.autodlResolution || node.autodlResolution || '768p竖'
        });
        seg.autodlTaskId = task.task_id;
        scheduleSave();
    }
    node.autodlLastTaskId = seg.autodlTaskId;
    const deadline = Date.now() + 30 * 60 * 1000;
    while(Date.now() < deadline){
        const task = await autodlFetch(`/api/autodl/tasks/${encodeURIComponent(seg.autodlTaskId)}`);
        node.autodlStatus = ({QUEUED:'排队中', RUNNING:'生成中', SUCCESS:'下载视频…', COMPLETED:'下载视频…'})[task.status] || task.status;
        render();
        if(['SUCCESS','COMPLETED'].includes(task.status)){
            const result = await autodlFetch('/api/autodl/download', {task_id:seg.autodlTaskId});
            delete seg.autodlTaskId;
            node.autodlStatus = '已完成';
            return result.urls;
        }
        if(task.status === 'FAILED'){
            delete seg.autodlTaskId;
    ...
    throw new Error('查询超时，任务编号已保存；再次点击生成可继续查询');
}
async function runMinimaxNode(nodeId){
    ...
    const autodlSegment = smartMinimaxEngine(node) === 'autodl' ? smartMinimaxSelectedSegment(node) : null;
    let runLog = smartMinimaxRunSnapshot(node);
    render();
    try {
        let urls = [];
        let resultKind = 'video';
        if(smartMinimaxEngine(node) === 'autodl'){
            urls = await runMinimaxAutoDL(node);
            runLog.settings.autodlTaskId = node.autodlLastTaskId || '';
        ...
        const seg = smartMinimaxSelectedSegment(node);
        out.forEach(item => smartMinimaxSetSegmentResult(node, autodlSegment || seg, item));
        ...
        if(autodlSegment) node.autodlStatus = readable;
...
shell.addEventListener('mousedown', e => {
    if(e.target.closest('.agent-panel,.agent-toggle')) return;
    if(!zoomPreviewState) return;
    if(e.button !== 0) return;
...
document.addEventListener('keydown', event => {
    if(event.target.closest?.('.agent-panel,.agent-toggle')) return;
...
window.onload = async () => {
    ...
    await loadAssetLibrary();
    await loadCanvas();
    window.CanvasAgent?.ui?.setOpen(true);
    syncApiKindToggleVisibility();
    render();
};
