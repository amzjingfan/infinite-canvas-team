...
const CODEX_DEFAULT_CHAT_MODELS = ['gpt-6-astra'];
...
const API_PROTOCOLS = ['openai', 'apimart', 'gemini', 'volcengine', 'runninghub', 'jimeng', 'codex', 'gemini-cli', 'autodl'];
...
function isFixedProvider(itemOrId){
    ...
    return id === 'modelscope' || id === 'runninghub' || id === 'volcengine' || id === 'autodl';
}
...
function syncEditor(){
    ...
    if(!item) return;
    const oldId = item.id;
    const isBuiltin = item.id === 'comfly' || item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'jimeng' || item.id === 'autodl';
...
function renderEditor(){
    const item = provider();
    if(!item) return;
    const isAutoDL = item.id === 'autodl';
    document.body.classList.toggle('show-autodl', isAutoDL);
    baseInput.readOnly = isAutoDL;
    ...
    if(protocolInput){
        ...
        protocolInput.value = item.id === 'runninghub'
            ...
            ? protocolValue
            : 'openai';
        protocolInput.disabled = isAutoDL || FIXED_PROTOCOL_PROVIDER_IDS.has(item.id) || Boolean(lockedApi);
    ...
    renderModels('video');
    if(isAutoDL){
        keyHint.textContent = '使用 AutoDL 的 ComfyUI 分组 API Key；保存后立即生效。';
    }
...
function renderModels(kind){
    ...
    if(item.id === 'autodl'){
        list.innerHTML = models.map(model => `<div class="model-row"><div class="model-id-field"><div class="model-display-name">MiniMax H3 · 1–15 秒 · 图片 / 音频参考</div><input value="${escapeAttr(model)}" readonly></div></div>`).join('');
        return;
    }
