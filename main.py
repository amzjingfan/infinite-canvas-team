...
# The bundled Windows Python omits the script directory from its isolated path.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from vinted_video import VintedVideoService
from canvas_agent import CanvasAgentStore, create_router as create_canvas_agent_router, validate_id as validate_agent_id
from canvas_image_workflow import CanvasImageWorkflow, IMAGE_SKILL_ID
from canvas_image_recipes import ImageRecipeStore, create_recipe_router
...
GITHUB_REPO_URL = "https://github.com/amzjingfan/infinite-canvas-team"
...
SKILL_CATALOG = SkillCatalog(os.path.join(os.path.expanduser('~'), '.agents', 'skills'), os.path.join(DATA_DIR, 'skills.json'))
app.include_router(create_skill_router(lambda: SKILL_CATALOG))
...
SUPPORTED_PROVIDER_PROTOCOLS = {"openai", "apimart", "gemini", "gemini-cli", "volcengine", "runninghub", "jimeng", "codex", "autodl"}
...
CODEX_DEFAULT_CHAT_MODELS = ["gpt-6-astra"]
...
CHAT_MODEL = os.getenv("CHAT_MODEL", "gpt-6-astra")
...
def provider_env_key_value(provider_id: str) -> str:
    ...
    if provider_id == "autodl":
        # An explicitly cleared platform key must not reactivate the earlier local key.
        if env_key in os.environ:
            return os.environ[env_key].strip()
        key = read_api_env_value(env_key) or os.getenv("AUTODL_API_KEY", "").strip()
        if not key and os.path.exists(AUTODL_KEY_FILE):
            with open(AUTODL_KEY_FILE, encoding="utf-8") as fh:
...
def merge_default_api_providers(providers, inject_missing=True):
    ...
    if not any(item.get("id") == "autodl" for item in merged):
        merged.append({
            "id": "autodl", "name": "AutoDL", "base_url": "https://autodl.art/api/v1/comfyui/comfyui_workflow",
            "protocol": "autodl", "enabled": True, "primary": False,
            "image_models": [], "chat_models": [], "video_models": ["minimax-h3"],
        })
...
def normalize_provider(item):
    ...
    if provider_id == "autodl":
        ...
        item = {**item, "image_models": [], "chat_models": [], "video_models": ["minimax-h3"]}
...
def prepare_chat_skills(payload):
    invocation = SKILL_CATALOG.invocation(payload.message, payload.skill_ids)
    if invocation['skills']:
        if payload.mode == 'image':
            raise HTTPException(422, '技能调用需要先由对话模型处理，请使用 Agent 模式。')
        payload.system_prompt = chat_system_prompt(payload) + '\n\n' + invocation['instructions']
    return invocation['skills']
...
def save_canvas(canvas):
    with CANVAS_LOCK:
        previous = load_canvas_any(canvas["id"]).get("updated_at", 0) if os.path.exists(canvas_path(canvas["id"])) else 0
        canvas["updated_at"] = max(now_ms(), int(previous or 0) + 1)
...
def prepare_canvas_agent_generation(kind, settings, prompt, media):
    """Local validation only; called before a paid step is marked submitting."""
    provider = settings["provider"]
    if not provider_env_key_value(provider):
        raise HTTPException(422, "所选生成平台未配置密钥，请在 API 设置中配置后再确认。")
    common = dict(prompt=prompt, provider_id=provider, model=settings["model"],
                  size=settings["size"], aspect_ratio=settings["aspect_ratio"], resolution=settings["resolution"])
    try:
        if provider == "autodl":
            return AutoDLVideoRequest(prompt=prompt, duration=settings["duration"],
                resolution=settings["resolution"] + ("竖" if settings["aspect_ratio"] == "9:16" else "横"),
                images=[m["url"] for m in media if m["kind"] == "image"],
                audios=[m["url"] for m in media if m["kind"] == "audio"])
        if kind == "image":
            return OnlineImageRequest(**common, quality=settings["quality"], n=1, reference_images=media)
        return CanvasVideoRequest(**common, duration=settings["duration"], images=media)
    except ValueError:
        limit = 10000 if provider == "autodl" else (ONLINE_IMAGE_PROMPT_MAX_LENGTH if kind == "image" else VIDEO_PROMPT_MAX_LENGTH)
        raise HTTPException(422, f"合并后的提示词或素材参数不符合生成接口要求（提示词上限 {limit} 字符），请缩短相连提示词并重新制定计划。")


canvas_agent_store = CanvasAgentStore(lambda: CANVAS_DIR, lambda canvas_id: load_canvas(canvas_id), lock=CANVAS_LOCK)
canvas_image_recipes = ImageRecipeStore(canvas_agent_store, resolve_media=lambda url: output_file_from_url(url))
app.include_router(create_recipe_router(canvas_image_recipes))
canvas_image_workflow = CanvasImageWorkflow(
    lambda: ImageCaseLibrary(SKILL_CATALOG.directory(IMAGE_SKILL_ID), ASSETS_DIR),
    lambda payload: run_canvas_agent_llm(payload))
app.include_router(create_canvas_agent_router(canvas_agent_store,
    run_llm=lambda payload: run_canvas_agent_llm(payload), get_provider=lambda provider: get_api_provider_exact(provider),
    prepare_generation=lambda kind, settings, prompt, media: prepare_canvas_agent_generation(kind, settings, prompt, media),
    generate_image=lambda request: build_online_image_result(request),
    generate_video=lambda request: canvas_video(request),
    autodl_submit=lambda request: autodl_submit(request), autodl_result=lambda task_id: autodl_result(task_id),
    autodl_download=lambda task_id: autodl_download(AutoDLDownloadRequest(task_id=task_id)),
    skill_invocation=lambda message, ids: SKILL_CATALOG.invocation(message, ids),
    image_workflow=canvas_image_workflow, recipe_store=canvas_image_recipes))
        ...
        for filename in os.listdir(CANVAS_DIR):
            ...
            try:
                ...
                if deleted_at and deleted_at < cutoff:
                    canvas_agent_store.purge(data["id"])
                    os.remove(path)
            except Exception:
                continue
...
def resolve_chat_provider(provider: str, model: str, ms_model: str, exact_provider=None):
    ...
    api_provider = exact_provider if exact_provider is not None else get_api_provider(provider or "")
...
def parse_agent_decision(raw_text, message, refs, has_previous_image, *, strict=False):
    ...
    if strict and (not isinstance(data, dict) or data.get('action') not in AGENT_ACTIONS
                   or not isinstance(data.get('prompt'), str) or not data['prompt'].strip()):
        raise ValueError('Skill planner returned an invalid decision')
    if not isinstance(data, dict):
        return heuristic
...
async def decide_chat_agent_action(payload, conversation, refs, *, require_skill_processing=False):
    ...
    if require_skill_processing and (is_codex_provider(provider_cfg) or is_gemini_cli_provider(provider_cfg)):
        raise HTTPException(422, '该平台的 Agent 生图暂不支持技能处理，请使用标准 API 对话平台，或仅在对话模式中调用技能。')
    ...
    try:
        async with httpx.AsyncClient(timeout=AI_REQUEST_TIMEOUT) as client:
            ...
            decision = parse_agent_decision(text_from_chat_response(raw), payload.message, refs, has_previous_image,
                                            strict=require_skill_processing)
            decision["router_model"] = model
            return decision
    except Exception as exc:
        if require_skill_processing:
            raise HTTPException(502, '技能处理失败，未提交图片生成。请检查对话模型后重试。') from None
...
@app.get("/api/config")
async def ai_config():
    preferred_chat_model = next((m for m in CHAT_MODELS if m == CHAT_MODEL), CHAT_MODELS[0] if CHAT_MODELS else CHAT_MODEL)
...
_vinted_service = None

def get_vinted_service():
    global _vinted_service
    if _vinted_service is None:
        _vinted_service = VintedVideoService(os.path.join(DATA_DIR, 'vinted'), get_api_provider_exact,
            provider_env_key_value, reference_to_data_url, output_path_for, output_url_for)
    return _vinted_service

@app.post('/api/vinted/operations/{operation}/resume')
async def resume_vinted_operation(operation: str):
    return await get_vinted_service().resume(operation)

@app.post("/api/canvas-video")
async def canvas_video(payload: CanvasVideoRequest):
    provider = get_api_provider(payload.provider_id)
    if provider['id'] == 'vinted' or urllib.parse.urlparse(provider.get('base_url', '')).hostname == 'vinted.cam':
        if not payload.vinted_operation_id:
            payload.vinted_operation_id = uuid.uuid4().hex
        return await get_vinted_service().generate(payload.model_dump())
    if is_jimeng_provider(provider):
...
async def canvas_llm(payload: CanvasLLMRequest):
    return await run_canvas_llm(payload)


async def run_canvas_agent_llm(payload):
    try:
        return await run_canvas_llm(CanvasLLMRequest(**payload), agent=True)
    except HTTPException as exc:
        if exc.status_code == 422:
            raise HTTPException(status_code=422, detail="引用素材无法完整读取或超过讨论上限（8 图/3 视频），请检查引用后重试。") from None
        raise HTTPException(status_code=502, detail="对话模型请求失败，请检查 API 设置；包含图片或视频时请改用支持视觉输入的模型。") from None
    except Exception:
        raise HTTPException(status_code=502, detail="对话模型请求失败，请检查 API 设置并重试。") from None


async def run_canvas_llm(payload: CanvasLLMRequest, *, agent=False):
    _provider = get_api_provider_exact(payload.provider) if agent else get_api_provider(payload.provider)
    ...
    if is_gemini_cli_provider(_provider):
        ...
        return {"text": text, "model": model, "raw_usage": None, "raw": raw}
    chat_base, chat_hdrs, model = resolve_chat_provider(payload.provider, payload.model, payload.ms_model,
                                                      exact_provider=_provider if agent else None)
    # 判断协议：APIMart 异步 vs 标准 OpenAI
    _llm_provider = _provider if agent else (get_api_provider(payload.provider) if payload.provider not in ("modelscope",) else {})
    ...
    if agent and (len(image_inputs) != len(payload.images) or len(video_inputs) != len(payload.videos)
                  or len(image_inputs) > 8 or len(video_inputs) > 3):
        raise HTTPException(status_code=422, detail="引用素材类型无效或超出讨论上限")
    if image_inputs or video_inputs:
        ...
        for img in image_inputs[:8]:
            ...
            if not ref_url:
                if agent:
                    raise HTTPException(status_code=422, detail="引用图片无法读取")
                continue
            if agent and not (ref_url.startswith(("http://", "https://", "data:image/"))):
                raise HTTPException(status_code=422, detail="引用图片地址无效")
            if agent and ref_url.startswith("data:image/"):
                try:
                    with Image.open(BytesIO(base64.b64decode(ref_url.split(",", 1)[1], validate=True))) as reference_image:
                        reference_image.load()
                except Exception:
                    raise HTTPException(status_code=422, detail="引用图片数据无效") from None
        ...
        for video in video_inputs[:3]:
            ...
            if frame_urls:
                ...
                if agent:
                    raise HTTPException(status_code=422, detail="引用视频无法提取关键帧")
    ...
    try:
        async with httpx.AsyncClient(timeout=AI_REQUEST_TIMEOUT) as client:
            req_body = {"model": model, "messages": upstream_messages}
            if agent and _provider.get("id") == "deepseek":
                # Agent stages require machine-readable output. DeepSeek's
                # documented JSON mode complements (not replaces) local schemas.
        ...
        if agent:
            raise HTTPException(status_code=502, detail="对话模型请求失败，请检查设置或改用支持视觉输入的模型。") from None
        body = exc.response.text or ""
...
async def purge_canvas(canvas_id: str):
    validate_agent_id(canvas_id)
    path = canvas_path(canvas_id)
    if os.path.exists(path):
        canvas = load_canvas_any(canvas_id)
        canvas_agent_store.purge(canvas["id"])
        os.remove(path)
    return {"ok": True}
...
async def chat(payload: ChatRequest, request: Request, x_user_id: str = Header(default="")):
    invoked_skills = prepare_chat_skills(payload)
    user_id = safe_user_id(x_user_id, request)
    conversation = (
    ...
    if invoked_skills:
        user_message['skills'] = invoked_skills
...
async def chat_agent(payload: ChatRequest, request: Request, x_user_id: str = Header(default="")):
    ...
    decision = await decide_chat_agent_action(payload, conversation, image_refs,
                                            require_skill_processing=bool(invoked_skills))
...
AUTODL_WORKFLOW = "minimax_h3_image_audio_to_video_v2_15s"
AUTODL_ROOT = "https://autodl.art/api/v1/comfyui/comfyui_workflow"
AUTODL_KEY_FILE = os.path.join(BASE_DIR, "API", "autodl.local.json")


class AutoDLSettings(BaseModel):
    api_key: str = Field(min_length=1)


class AutoDLVideoRequest(BaseModel):
    prompt: str = Field(min_length=1, max_length=10000)
    duration: int = Field(default=5, ge=1, le=15, strict=True)
...
def autodl_headers():
    key = provider_env_key_value("autodl")
    if not key:
        raise HTTPException(400, "请先配置 AutoDL API 密钥（ComfyUI 分组）")
    return {"Authorization": key, "Content-Type": "application/json"}


@app.post("/api/autodl/settings")
async def autodl_save_settings(payload: AutoDLSettings):
    if not payload.api_key.strip():
        raise HTTPException(400, "密钥不能为空")
    update_env_values({provider_key_env("autodl"): payload.api_key.strip()})
    return {"ok": True}


async def autodl_reference_url(value, kind):
    ...
    if parsed.scheme in {"http", "https"}:
        ...
        try:
            ...
            private = host.lower() == "localhost"
        if private:
            raise HTTPException(400, "AutoDL 无法读取本机或局域网链接，请从素材库添加本地文件")
        return value
    prefixes = ("audio/", "video/") if kind == "audio" else ("image/",)
    path = local_media_path_for_cloud_upload(value, prefixes)
    allowed = {"image": {".jpg", ".jpeg", ".png", ".webp"}, "audio": {".mp3", ".wav", ".mp4", ".flac"}}
    if os.path.splitext(path)[1].lower() not in allowed[kind]:
        raise HTTPException(400, f"AutoDL 不支持该{kind}文件格式")
    uploaded = await upload_video_to_litterbox(path, value)
    return uploaded["url"]


async def autodl_request(method, suffix, body=None):
    headers = autodl_headers()
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            response = await client.request(method, AUTODL_ROOT + suffix, headers=headers, json=body)
        response.raise_for_status()
        raw = response.json()
    except (httpx.HTTPError, ValueError) as exc:
        raise HTTPException(502, "AutoDL 请求失败；已提交的任务请继续查询，不要重复生成") from exc
    if raw.get("code") != "Success":
        raise HTTPException(502, f"AutoDL：{raw.get('msg') or raw.get('code') or '未知错误'}")
    return raw.get("data") or {}


@app.post("/api/autodl/tasks")
async def autodl_submit(payload: AutoDLVideoRequest):
    autodl_headers()
    if not payload.prompt.strip():
        raise HTTPException(400, "请输入提示词")
    if payload.resolution not in {"480p竖", "768p竖", "480p横", "768p横"}:
        raise HTTPException(400, "请选择 AutoDL 支持的分辨率")
    body = {"prompt": payload.prompt, "duration": payload.duration, "resolution": payload.resolution}
    if payload.seed is not None:
        body["seed"] = payload.seed
    for kind, refs in (("image", payload.images), ("audio", payload.audios)):
        for index, ref in enumerate(refs):
            body[f"ref_{kind}_{index}"] = await autodl_reference_url(ref, kind)
    data = await autodl_request("POST", "/" + AUTODL_WORKFLOW, body)
    if not data.get("task_id"):
        raise HTTPException(502, "AutoDL 未返回任务编号，请到平台检查任务后再操作")
    return {"task_id": data["task_id"], "status": data.get("status", "QUEUED")}


@app.get("/api/autodl/tasks/{task_id}")
async def autodl_result(task_id: str):
    if not re.fullmatch(r"[a-zA-Z0-9_-]+", task_id):
        raise HTTPException(400, "任务编号无效")
    data = await autodl_request("GET", "/result/" + task_id)
    ...
    if status in {"SUCCESS", "COMPLETED"}:
        ...
        if not urls:
            raise HTTPException(502, "AutoDL 任务完成，但未返回视频")
    return {"task_id": task_id, "status": status, "urls": urls, "message": data.get("message") or ""}


class AutoDLDownloadRequest(BaseModel):
    task_id: str


@app.post("/api/autodl/download")
async def autodl_download(payload: AutoDLDownloadRequest):
    result = await autodl_result(payload.task_id)
    if not result["urls"]:
        raise HTTPException(400, "视频尚未生成完成")
    return {"urls": [await save_remote_video_to_output(url) for url in result["urls"]]}


from team_update_api import install_team_updates
install_team_updates(app, BASE_DIR, lambda: bool(QUEUE) or any(
    task.get('status') in ('queued', 'running', 'pending') for task in CANVAS_TASKS.values()))
